...
Pathlist createpaths(int pointnum, Polygon pol){
    Pathlist temp;
    if(pointnum<2){
        std::cout<<"Insuffisient amount of point allowed in a path\nThe amount of points is:"<<pointnum<<std::endl;
        return temp;
    }
    Vector Path;
    int p=0;
    for(int i=0;i<pol.size();i++){
        std::cout<<"i= "<<i<<" At point:"<<pol[i]<<std::endl;
        Path.clear();
        for(int j=0;j<pointnum;j++){
            ...
            std::cout<<"i: "<<i<<" j: "<<j<<" - "<<pol[p]<<std::endl;
